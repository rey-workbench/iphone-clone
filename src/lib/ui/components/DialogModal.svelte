...
{#if dialogState.isOpen && dialogState.options}
  <!-- Backdrop -->
  <div
    class="absolute inset-0 z-[10000] bg-black/40 flex items-center justify-center p-8 backdrop-blur-[2px] animate-[fadeIn_0.2s_ease]"
  >
    <!-- Dialog Box -->
    <div
      class="bg-[#2C2C2E]/95 backdrop-blur-xl w-full max-w-[270px] rounded-[14px] overflow-hidden flex flex-col shadow-2xl animate-[scaleIn_0.2s_cubic-bezier(0.175,0.885,0.32,1.275)]"
    >
      <!-- Content Area -->
      ...
      {#if dialogState.options.cancelText && dialogState.options.confirmText}
        <div class="flex border-t border-white/10 h-[44px]">
          <button
