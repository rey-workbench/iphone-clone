...
{#if shellState.isControlCenterOpen || shellState.isControlCenterDragging}
  ...
  <div 
    ...
    style="transform: translateY(calc({baseTransform} + {shellState.controlCenterDragY}px)); 
           {shellState.isControlCenterDragging ? 'transition: none;' : ''}"
  ...
  >
    ...
    <!-- Layout Grid -->
    <div class="relative z-10 p-6 pt-16 h-full flex flex-col gap-4 animate-[slideDown_0.35s_cubic-bezier(0.23,1,0.32,1)]" style={!shellState.isControlCenterOpen && !shellState.isControlCenterDragging ? 'animation: none' : ''}>
