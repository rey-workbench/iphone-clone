  ...
  <div class="flex-1 overflow-y-auto px-4 pb-10">
    ...
    <div class="bg-ios-bg2 rounded-xl mb-5 overflow-hidden">
      {#each toggleItems as item, i}
        <button class="flex items-center gap-3 py-[11px] px-4 w-full border-none bg-transparent cursor-pointer text-white text-left" onclick={() => item.toggle && appState.toggle(item.id)}>
    ...
    <div class="bg-ios-bg2 rounded-xl mb-5 overflow-hidden">
      {#each appState.general as item, i}
        <button class="flex items-center gap-3 py-[11px] px-4 w-full border-none bg-transparent cursor-pointer text-white text-left" onclick={() => { if(item.id) activeView = item.id; }}>
          <div class="w-7 h-7 rounded-md flex items-center justify-center shrink-0" style="background:{item.bg}">
            <item.icon size={16} color="white" />
          </div>
          <div class="flex-1 text-[17px]">{item.label}</div>
        ...
        {#if i < appState.general.length - 1}<div class="h-px bg-ios-sep ml-[52px]"></div>{/if}
      {/each}
    </div>
    <!-- Settings -->
    <div class="bg-ios-bg2 rounded-xl mb-5 overflow-hidden">
