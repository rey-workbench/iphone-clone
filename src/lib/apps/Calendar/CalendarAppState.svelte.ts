...
export class CalendarAppState extends BaseGlobalState {
  ...
  monthName = $derived(this.selectedDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric' }));
  ...
  isToday(day: number) {
      return this.today.getDate() === day && this.today.getMonth() === this.month && this.today.getFullYear() === this.year;
  }
