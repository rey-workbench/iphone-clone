...
import type { IBtn } from '$lib/framework/types';
...
export class CalculatorAppState extends BaseGlobalState {
	...
	buttons: IBtn[][] = [
		...
		[
			{ label: '0', type: 'zero', value: '0', span: 2 },
	...
	press(btn: IBtn) {
		if (btn.type === 'number' || btn.type === 'zero') {
			...
			if (this.waiting) {
				this.display = btn.value === '.' ? '0.' : btn.value;
				this.waiting = false;
			} else {
				this.display =
					this.display === '0' && btn.value !== '.' ? btn.value : this.display + btn.value;
			}
			this.activeOp = null;
		} else if (btn.type === 'operator') {
			const cur = parseFloat(this.display);
			if (btn.value === '=') {
				if (this.op && this.prevVal !== null) {
					this.display = String(this.calc(this.prevVal, cur, this.op));
					this.prevVal = null;
					this.op = null;
				}
				this.activeOp = null;
			} else {
				if (this.op && this.prevVal !== null && !this.waiting) {
					const r = this.calc(this.prevVal, cur, this.op);
