import { mailDb, MailDBKey } from '$lib/framework/db';
import { SyncState } from '$lib/os/state/SyncState.svelte';

import type { IEmail } from '$lib/framework/types';
...
export class MailAppState extends SyncState<IEmail[]> {
	...
	constructor() {
		super(mailDb, MailDBKey.MAIL_INBOX, [], async () => {
			const { MailApiClient } = await import('$lib/framework/api/services/MailApiClient');
			const data = await MailApiClient.getComments();
			if (data && Array.isArray(data)) {
				return data.map(
					(item: { id: number; name: string; email: string; body: string }, i: number) => ({
						id: String(item.id),
