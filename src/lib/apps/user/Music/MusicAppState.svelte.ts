...
import { MusicApiClient } from '$lib/framework/api/services/MusicApiClient';
import {
	EMusicItemType,
	EMusicAction,
	type IMusicTrack,
...
export class MusicAppState extends BaseGlobalState {
	...
	handleSearchInput() {
		if (this.searchQuery.trim().length > 1) {
			...
			this.searchTimeout = setTimeout(async () => {
				try {
					const r = await MusicApiClient.search({
						action: EMusicAction.SUGGESTIONS,
						q: this.searchQuery
					});
	...
	async play(t: IMusicTrack) {
		if (
			t.type === EMusicItemType.PLAYLIST ||
			t.type === EMusicItemType.ALBUM ||
			t.type === EMusicItemType.ARTIST
		) {
			try {
				const r = await MusicApiClient.search({
					action: EMusicAction.PLAYLIST_TRACKS,
					q: t.id,
					type: t.type
				});
	...
	async fetchUpNext() {
		...
		try {
			...
			dialogGlobalState.show({
				title: 'Up Next Error',
				message: err.message || 'Failed to load upcoming tracks.',
				confirmText: 'OK'
			});
		}
	}
