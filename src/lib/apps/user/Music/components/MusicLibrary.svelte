...
{#snippet trackItem(t: import('$lib/framework/types').IMusicTrack, i: number, isSearch: boolean)}
	<button
		data-id={t.id || t.name}
		class="flex gap-3 p-2 px-3 w-full border-none bg-transparent cursor-pointer text-left text-white items-center"
		onclick={isSearch ? handleSearchResultPlay : handlePlay}
	>
...
<div class="flex-1 overflow-y-auto px-4 pt-13.5 pb-5">
	{#if state.activeTab === 'listen_now' || state.activeTab === 'browse' || state.activeTab === 'radio' || state.activeTab === 'library'}
		...
		{#if state.tracks.length === 0}
			<div class="mb-5">
				<h2 class="text-[22px] font-bold text-white mb-3">
