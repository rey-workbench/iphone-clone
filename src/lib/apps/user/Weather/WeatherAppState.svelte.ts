	...
	constructor() {
		super(settingsDb, 'app_weather', defaultData, async () => {
				...
				const timeStr =
					i === 0
						...
						: new Date(data.hourly.time[i])
								...
								.replace(' ', '');
				const isNight =
					new Date(data.hourly.time[i]).getHours() < 6 ||
					new Date(data.hourly.time[i]).getHours() > 18;
				hourly.push({
					time: timeStr,
					temp: Math.round(data.hourly.temperature_2m[i]),
					icon: WeatherAppState.getIconStatic(data.hourly.weather_code[i], isNight)
				});
			}
			...
			for (let i = 0; i < 7; i++) {
				...
				const dayStr =
					i === 0 ? 'Today' : dateObj.toLocaleDateString('en-US', { weekday: 'short' });
				daily.push({
					day: dayStr,
					high: Math.round(data.daily.temperature_2m_max[i]),
					low: Math.round(data.daily.temperature_2m_min[i]),
					icon: WeatherAppState.getIconStatic(data.daily.weather_code[i])
				});
			}
			...
			const w = {
				...
				tiles: [
					{
						title: 'UV INDEX',
						value: Math.round(data.daily.uv_index_max[0] || 0).toString(),
						desc: 'Moderate'
					},
					...
					{
						title: 'PRESSURE',
						value: Math.round(data.current.surface_pressure).toString(),
						desc: 'hPa'
					}
				]
			};
