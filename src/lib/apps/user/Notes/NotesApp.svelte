...
<AppContainer appName="Notes" paddingBottom="pb-0">
	{#if state.selectedNote}
		<div class="flex-1 flex flex-col">
			<div class="flex justify-between items-center px-4 py-2 border-b border-ios-sep">
				<button
					class="bg-transparent border-none text-[#FF9F0A] text-[17px] cursor-pointer flex items-center"
					onclick={handleGoBack}
				>
					<ChevronLeft size={20} class="mr-1" /> Notes
				</button>
				<button
					class="bg-transparent border-none text-[#FF9F0A] cursor-pointer"
			...
			<div class="flex-1 p-4 flex flex-col gap-2">
				<input
					class="bg-transparent border-none text-white text-2xl font-bold outline-none w-full"
	...
	{:else}
		<div class="flex-1 flex flex-col overflow-hidden">
				...
				{#if state.isLoading}
					<IOSList>
						{#each Array(4) as _, i (i)}
							<div
								class="w-full text-left p-3 px-4 flex flex-col gap-1.5 {i < 3
									? 'border-b border-ios-sep'
									: ''}"
							>
				...
				{:else}
					<IOSList>
						{#each state.notes as note, i (i)}
							<button
								data-id={note.id}
								class="w-full text-left p-3 px-4 bg-ios-bg2 border-none text-white cursor-pointer {i <
							...
							>
								...
								<div class="flex gap-2 text-[13px] text-ios-label2">
									<span
										>{state.fmtDate(
											typeof note.date === 'string' ? new Date(note.date) : note.date
										)}</span
									>
			...
			<div
				class="flex justify-between items-center px-4 pt-2 pb-8 bg-[rgba(30,30,30,0.95)] border-t border-ios-sep"
			>
				<span></span>
				<span class="text-[13px] text-ios-label2">{state.notes.length} Notes</span>
				<button
					class="w-11 h-11 bg-transparent border-none text-[#FF9F0A] cursor-pointer flex justify-end items-center"
