<script lang="ts">
	import { clockAppState } from '$lib/apps/user/Clock/ClockAppState.svelte';
	...
	const tabs: {
		id: 'worldClock' | 'alarm' | 'stopwatch' | 'timer';
		label: string;
		icon: typeof Globe;
	}[] = [
...
<div class="h-full pt-13.5 pb-0 bg-black flex flex-col">
	<div class="flex-1 overflow-y-auto px-4">
				...
				{#each clocks as wc, i (i)}
					<div class="flex items-center justify-between p-3 px-4">
						<div class="flex flex-col gap-0.5">
							<span class="text-[13px] text-ios-label2">{wc.label}</span><span
								class="text-[17px] text-white font-medium">{wc.city}</span
							>
						</div>
		...
		{:else if tab === 'alarm'}
			...
			<div class="bg-ios-bg2 rounded-xl overflow-hidden">
				{#each alarms as alarm, i (i)}
					<div class="flex items-center justify-between p-3 px-4">
						<div class="flex flex-col gap-0.5">
							<span class="text-[48px] font-extralight text-white">{alarm.time}</span><span
								class="text-[13px] text-ios-label2">{alarm.label}, {alarm.days}</span
							>
						</div>
						<button
							data-id={alarm.id}
							class="w-12.75 h-7.75 rounded-2xl relative transition-colors border-none cursor-pointer shrink-0 {alarm.enabled
								? 'bg-ios-green'
								: 'bg-[#39393D]'}"
						...
						>
							<div
								class="w-6.75 h-6.75 rounded-full bg-white absolute top-0.5 left-0.5 transition-transform shadow-md {alarm.enabled
									? 'translate-x-5'
									: ''}"
							></div>
						</button>
		...
		{:else if tab === 'stopwatch'}
			...
			<div class="text-[72px] font-extralight text-white text-center py-15 tabular-nums">
				{fmtSW(swMs)}
			</div>
			<div class="flex justify-center gap-7.5 px-5">
				<button
					class="w-20 h-20 rounded-full border-2 border-ios-gray bg-ios-gray/20 text-white text-[16px] font-medium cursor-pointer"
					onclick={resetSW}>Reset</button
				>
				<button
					class="w-20 h-20 rounded-full border-2 cursor-pointer text-[16px] font-medium {swRunning
						? 'border-ios-red bg-ios-red/20 text-ios-red'
						: 'border-ios-green bg-ios-green/20 text-ios-green'}"
					onclick={toggleSW}>{swRunning ? 'Stop' : 'Start'}</button
				>
			</div>
		{:else}
			...
			<div class="flex justify-center gap-7.5 px-5">
				<button
					class="w-20 h-20 rounded-full border-2 border-ios-gray bg-ios-gray/20 text-white text-[16px] font-medium cursor-pointer"
					onclick={resetTimer}>Cancel</button
				>
				<button
					class="w-20 h-20 rounded-full border-2 cursor-pointer text-[16px] font-medium {timerRunning
						? 'border-ios-red bg-ios-red/20 text-ios-red'
						: 'border-ios-green bg-ios-green/20 text-ios-green'}"
					onclick={toggleTimer}>{timerRunning ? 'Pause' : 'Start'}</button
				>
			</div>
		{/if}
	</div>
	<div
		class="flex bg-[rgba(30,30,30,0.95)] backdrop-blur-[20px] border-t border-ios-sep pt-1.5 pb-8 shrink-0 justify-around"
	>
		{#each tabs as t (t.id || t)}
			<button
				data-id={t.id}
				class="flex-1 flex flex-col items-center gap-1 border-none bg-transparent cursor-pointer py-1 {tab ===
				t.id
					? 'text-[#FF9F0A]'
					: 'text-ios-label2'}"
				onclick={setTab}
