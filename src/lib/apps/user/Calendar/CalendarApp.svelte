	...
	<div class="flex-1 overflow-y-auto px-4 pb-10">
		<AppHeader title={state.monthName} paddingClass="px-1 py-2 pb-4">
			{#snippet rightSlot()}
				<div class="flex gap-4">
					<button
						class="bg-transparent border-none text-ios-blue text-lg cursor-pointer flex items-center justify-center"
						onclick={prevMonth}
					>
						<ChevronLeft size={24} />
					</button>
		...
		<div class="grid grid-cols-7 gap-y-1 mb-6">
			...
			{#each Array(state.daysInMonth) as _, i (i)}
				...
				<button
					data-day={day}
					class="w-10 h-10 mx-auto rounded-full border-none flex items-center justify-center text-[16px] cursor-pointer transition-colors
          {state.isToday(day)
		...
		<IOSList>
			<div
				class="text-[13px] font-semibold text-ios-label2 tracking-wider p-3 px-4 border-b border-ios-sep"
			>
				EVENTS
			</div>
			{#each state.events as evt, i (i)}
				<div class="flex gap-3 p-3 px-4 items-start">
					...
					<div class="flex flex-col gap-0.5">
						<span class="text-[17px] font-medium text-white">{evt.title}</span><span
							class="text-[13px] text-ios-label2">{evt.time}</span
						>
					</div>
				</div>
