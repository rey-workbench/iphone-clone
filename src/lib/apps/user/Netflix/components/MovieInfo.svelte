...
<div class="px-3 pt-2 flex flex-col gap-3 pb-12">
	...
	<div class="flex items-center gap-1">
		<img src="/assets/icons/netflix-brand-logo.png" alt="N" class="h-4 w-auto object-contain" />
	...
	<div class="flex items-center gap-2 text-xs text-gray-400 font-medium">
		{#if media.vote_average}
			<span class="text-[#46D369] font-bold">{Math.round(media.vote_average * 10)}% match</span>
		{/if}
		<span>2020</span>
		<span class="px-1.5 py-0.5 bg-[#333] text-gray-300 rounded-sm text-[10px] font-bold">18+</span>
		{#if isTvShow}<span
				>{netflixState.details.seasons.length > 0
					? netflixState.details.seasons.length
					: media.seasons || 1} Seasons</span
			>{/if}
		<span class="border border-gray-500 px-1 rounded-sm text-[10px] font-bold">HD</span>
		<span class="border border-gray-500 px-1 rounded-sm text-[10px] font-bold">AD</span>
	</div>
	...
	<div class="flex items-center gap-10 mt-4 px-4">
		<button
			class="flex flex-col items-center gap-1.5 cursor-pointer text-[#B3B3B3] hover:text-white bg-transparent border-none p-0"
		>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				width="24"
	...
	<div class="flex items-center gap-2 mt-6 mb-1 overflow-x-auto no-scrollbar">
