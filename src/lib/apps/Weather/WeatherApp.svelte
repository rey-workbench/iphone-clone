...
<div class="h-full pt-[54px] pb-5 relative overflow-hidden">
  ...
  <div class="relative z-10 h-full overflow-y-auto ">
    <div class="text-center px-5 ">
      ...
      <p class="text-lg text-white font-medium">H:{state.w.high}° L:{state.w.low}°</p>
    </div>
      ...
      <div class="rounded-[14px] p-3 px-4 bg-white/12 backdrop-blur-[20px] border border-white/20">
        ...
        {#each state.w.daily as d, i}
          <div class="flex items-center py-1.5 gap-2">
            ...
            <div class="flex-1 h-1 bg-white/15 rounded-sm relative"><div class="absolute h-full rounded-sm bg-linear-to-r from-[#4CD964] via-[#FFCC00] to-[#FF6B35]" style={state.bar(d.low, d.high)}></div></div>
            <span class="text-[16px] text-white w-8 text-right">{d.high}°</span>
          </div>
          {#if i < state.w.daily.length - 1}<div class="h-px bg-white/15"></div>{/if}
        {/each}
      </div>
