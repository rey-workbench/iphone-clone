  ...
  {#if state.selected}
    <div class="flex-1 flex flex-col">
      <div class="flex items-center px-4 py-2 border-b border-ios-sep">
        <button class="bg-transparent border-none text-ios-blue text-[17px] cursor-pointer flex items-center" onclick={closeEmail}>
      ...
      <div class="bg-ios-bg2 rounded-xl overflow-hidden divide-y divide-ios-sep">
        ...
        {:else}
          {#each state.emails as email, i (i)}
            <button data-index={i} class="flex gap-3 p-3 px-4 w-full border-none bg-transparent cursor-pointer text-left text-white items-start" onclick={openEmail}>
