<script lang="ts">
  ...
  const inbox = [
    { name: 'Buddy Bard', avatar: '🤖', color: '#007AFF', lastMsg: "Hey! I'm your AI assistant.", time: '12:00 PM', unread: true },
    { name: 'Tim Cook', avatar: 'TC', color: '#8E8E93', lastMsg: 'Big announcements coming.', time: 'Yesterday', unread: false },
  ];
  ...
  {#if chatView}
    <div class="flex-1 flex flex-col">
      <div class="flex items-center justify-between px-4 py-2 border-b border-ios-sep">
        <button class="bg-transparent border-none text-ios-blue text-[17px] cursor-pointer" onclick={() => chatView = false}>← Messages</button>
      ...
      <div class="flex items-center gap-2 px-3 py-2  border-t border-ios-sep bg-[rgba(30,30,30,0.95)]">
        ...
        {#if inputText}
          <button class="w-[30px] h-[30px] rounded-full bg-ios-blue border-none text-white text-[16px] font-bold cursor-pointer flex items-center justify-center" onclick={send}>↑</button>
        {/if}
      </div>
    </div>
  {:else}
      ...
      <div class="bg-ios-bg2 rounded-xl overflow-hidden">
        {#each inbox as convo}
          <button class="flex gap-3 p-3 px-4 w-full border-none bg-transparent cursor-pointer text-white text-left border-b border-ios-sep last:border-b-0" onclick={() => { if (convo.name === 'Buddy Bard') chatView = true; }}>
            <div class="w-[45px] h-[45px] rounded-full flex items-center justify-center text-[16px] font-semibold text-white shrink-0" style="background:{convo.color}">{convo.avatar}</div>
