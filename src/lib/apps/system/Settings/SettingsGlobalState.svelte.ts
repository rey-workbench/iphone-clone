...
import { settingsDb, SettingsDBKey } from '$lib/framework/db';
import { PersistedState } from '$lib/os/state/PersistedState.svelte';
...
export class SettingsAppState extends BaseGlobalState {
	...
	toggle(id: string) {
		...
		if (id === 'airplane')
			settingsGlobalState.data.airplaneMode = !settingsGlobalState.data.airplaneMode;
		if (id === 'wifi') settingsGlobalState.data.wifi = !settingsGlobalState.data.wifi;
		if (id === 'bluetooth')
			settingsGlobalState.data.bluetooth = !settingsGlobalState.data.bluetooth;
