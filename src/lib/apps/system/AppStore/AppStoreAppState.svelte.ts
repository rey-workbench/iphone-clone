...
import { appstoreDb, AppStoreDBKey } from '$lib/framework/db';
import { SyncState } from '$lib/os/state/SyncState.svelte';

import type { TAppStoreTabId, IAppStoreData, IAppStoreProduct } from '$lib/framework/types';
...
export class AppStoreAppState extends SyncState<IAppStoreData | null> {
	...
	constructor() {
		super(appstoreDb, AppStoreDBKey.APPSTORE_PRODUCTS, null, async () => {
			const { AppStoreApiClient } = await import('$lib/framework/api/services/AppStoreApiClient');
