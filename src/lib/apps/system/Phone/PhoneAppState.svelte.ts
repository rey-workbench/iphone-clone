...
import type { TPhoneTabId, ICallHistoryEntry, IContact } from '$lib/framework/types';
...
export class PhoneAppState extends BaseGlobalState {
	...
	constructor() {
		...
		if (typeof window !== 'undefined') {
			window.addEventListener('reyos:call_ended', () => this.loadRecents());
		}
	}

	async loadRecents() {
		this.loadingRecents = true;
		const { getCallHistory } = await import('$lib/framework/db');
	...
	updateContacts(users: import('$lib/framework/types').IUser[]) {
