<script lang="ts">
	...
	const tabItems: {
		id: 'favorites' | 'recents' | 'contacts' | 'keypad' | 'voicemail';
		label: string;
		icon: Component;
	}[] = [
		...
		{#if state.tab === 'keypad'}
			<div class="flex flex-col items-center py-5">
						...
						<div class="flex gap-5.5">
							{#each row as k (k.n || k.s)}
								...
								>
									...
									<span class="text-[10px] font-medium tracking-widest text-ios-label2 h-3.5"
										>{k.s}</span
									>
								</button>
							{/each}
				...
				<div class="flex items-center justify-center gap-15 py-5">
					...
					{#if state.dialNumber}
						<button
							class="bg-transparent border-none text-2xl cursor-pointer text-ios-label2"
		...
		{:else}
			<div class="px-4">
				<h1 class="text-[34px] font-bold text-white px-1 py-2 pb-4">
					{state.tab === 'favorites' ? 'Favorites' : 'Voicemail'}
				</h1>
