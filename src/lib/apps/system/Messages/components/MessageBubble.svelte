<script lang="ts">
	import type { IChatMessage } from '$lib/framework/types';
	const {
		msg,
		isLastInGroup,
		isFirstInGroup,
...
<div
	class="flex items-end {msg.isUser ? 'justify-end pr-2' : 'pl-2'} {isLastInGroup
		? 'mb-2'
		: ''} {isFirstInGroup ? 'mt-1' : ''} {isSelectionMode ? 'pl-10' : ''} relative"
>
	{#if isSelectionMode}
		<button
			class="absolute left-2 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full border border-ios-sep bg-transparent flex items-center justify-center p-0 cursor-pointer {isSelected
				? 'bg-ios-blue border-none'
				: ''}"
			data-id={msg.id}
	...
	<div
		class="max-w-[75%] px-3.5 py-2 rounded-[18px] text-[17px] leading-[1.3] whitespace-pre-wrap wrap-break-word relative
           ...
           {isLastInGroup && msg.isUser && !isSelectionMode
			? `${tailRightClasses} rounded-br-md`
			: ''}
           {isLastInGroup && !msg.isUser && !isSelectionMode
			? `${tailLeftClasses} rounded-bl-md`
			: ''}"
		data-id={msg.id}
