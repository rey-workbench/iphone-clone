<script lang="ts">
	...
	const { appState, usersGlobalState, closeChat }: Props = $props();
</script>

<div
	class="relative flex items-center justify-between px-3 py-2 border-b border-[#222] h-22.5 bg-black/80 backdrop-blur-xl z-20"
>
	<!-- Back Button -->
	<button
	...
	>
		...
		{#if appState.totalUnread > 0}
			<span
				class="h-5.5 min-w-5.5 px-1.5 rounded-full bg-ios-blue text-white flex items-center justify-center text-[13px] font-semibold"
	...
	<!-- Profile / Name -->
	<div class="absolute left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 mt-1">
		<div
			class="w-12.5 h-12.5 rounded-full bg-ios-bg2 text-white flex items-center justify-center text-[22px] font-semibold"
		...
		<span class="text-[12px] font-medium text-white/90 flex items-center gap-0.5">
