<script lang="ts">
	import ContextMenu, { type ContextMenuItem } from '$lib/framework/ui/ContextMenu.svelte';
...
<div class="fixed inset-0 z-100 flex items-center justify-center backdrop-blur-xl bg-black/40">
	...
	<div
		...
		style:right={activeContextMenu.isUser
			...
			: undefined}
		style:left={!activeContextMenu.isUser ? `${activeContextMenu.rect.left}px` : undefined}
	>
	...
	<ContextMenu
		...
		menuRight={activeContextMenu.isUser
			? Math.max(10, activeContextMenu.winWidth - activeContextMenu.rect.right)
			: undefined}
