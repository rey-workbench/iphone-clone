<script lang="ts">
	...
	import { usersGlobalState } from '$lib/os/state';
	import Skeleton from '$lib/framework/ui/Skeleton.svelte';
	import { type ContextMenuItem } from '$lib/framework/ui/ContextMenu.svelte';
	...
	function getMenuItems(): ContextMenuItem[] {
		...
		return [
			{ label: 'Reply', icon: Reply, onClick: () => {} },
			{
				label: 'Copy',
				icon: Copy,
				onClick: () => navigator.clipboard.writeText(activeContextMenu!.content)
			},
				...
				? [
						{
							...
							onClick: () => appState.deleteMessage(activeContextMenu!.msgId)
						}
					]
				: []),
			{
	...
	function handleDeleteSelected() {
		appState.messages
			...
			.forEach((m) => appState.deleteMessage(m.id));
		...
		<div class="flex-1 flex flex-col min-h-0 bg-black">
			...
			>
				{#each appState.messages as msg, i (msg.id || i)}
					{@const isLastInGroup =
						i === appState.messages.length - 1 || appState.messages[i + 1].isUser !== msg.isUser}
			...
			{#if isSelectionMode}
				...
				>
					...
					<button
						class="bg-ios-bg2 border-none rounded-full px-4 py-1.5 text-white font-semibold cursor-pointer"
						>Forward</button
					>
				</div>
			{:else}
				<MessagesInputBar {appState} {adjustTextareaHeight} {handleInputKeydown} {handleSend} />
			{/if}
		</div>
	{:else}
