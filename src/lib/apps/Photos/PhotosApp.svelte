...
<div class="h-full pt-[54px] pb-0 bg-black flex flex-col">
	{#if state.selectedPhoto}
		<div class="flex-1 flex flex-col">
			...
			<div class="flex-1 flex flex-col items-center justify-center bg-black p-4 gap-2">
				<img
					src={state.selectedPhoto.download_url}
					alt="Full view"
	...
	{:else}
			...
			{:else}
				<div class="grid grid-cols-3 gap-0.5 px-0.5">
					{#each state.photos as photo (photo.id || photo)}
						<button
							data-id={photo.id}
							class="aspect-square overflow-hidden border-none p-0 cursor-pointer bg-ios-bg3"
		...
		<div
			class="flex bg-[rgba(30,30,30,0.95)] backdrop-blur-[20px] border-t border-ios-sep pt-1.5 pb-8 shrink-0"
		>
			{#each [{ id: 'library', icon: Image, label: 'Library' }, { id: 'foryou', icon: Heart, label: 'For You' }, { id: 'albums', icon: Folder, label: 'Albums' }, { id: 'search', icon: Search, label: 'Search' }] as { id, icon: IconComponent, label } (id)}
				<button
					data-tab={id}
					class="flex-1 flex flex-col items-center gap-0.5 border-none bg-transparent cursor-pointer py-1 {state.tab ===
					id
						? 'text-ios-blue'
						: 'text-ios-label2'}"
