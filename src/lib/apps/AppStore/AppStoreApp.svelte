<script lang="ts">
  ...
  const tabItems: { id: TAppStoreTabId; label: string; icon: typeof Search }[] =
    [
  ...
  const setTab = (e: MouseEvent) => {
    ...
    state.tab = btn.dataset.id as TAppStoreTabId;
  };
</script>

<AppContainer appName="App Store" paddingBottom="pb-0">
  <div class="flex-1 overflow-y-auto px-4">
    <div class="px-1 py-2 pb-4">
      <span
        class="text-[13px] text-ios-label2 uppercase tracking-wider font-semibold"
      ...
      {:else}
        {#each state.featured as app (app.id || app)}
          <div class="rounded-2xl overflow-hidden bg-ios-bg2">
            ...
            <div class="p-3">
              <div
                class="text-[11px] text-ios-label2 uppercase tracking-wider font-semibold"
              >
                {app.cat}
              </div>
              <div class="text-[20px] font-bold text-white">{app.name}</div>
              <div class="text-[13px] text-ios-label2">{app.dev}</div>
            </div>
          </div>
        {/each}
      {/if}
    ...
    {:else}
      <IOSList>
        {#each state.topApps as app, i (i)}
          <div class="flex items-center gap-3 p-2.5 px-4">
            <span class="text-[17px] text-ios-label2 w-4 text-right font-medium"
              >{app.rank}</span
            >
            <div
              class="w-12 h-12 rounded-xl overflow-hidden bg-ios-fill flex items-center justify-center shrink-0"
            >
              <img
                src={app.img}
                alt={app.name}
            ...
            <div class="flex-1 min-w-0">
              <div class="text-[17px] text-white truncate">{app.name}</div>
              <div class="text-[13px] text-ios-label2">{app.cat}</div>
            </div>
            <button
              class="px-4 py-1.5 rounded-full bg-ios-fill border-none text-ios-blue text-[15px] font-semibold cursor-pointer"
              >GET</button
            >
          </div>
          {#if i < state.topApps.length - 1}<div
              class="h-px bg-ios-sep ml-19"
            ></div>{/if}
        {/each}
      </IOSList>
    {/if}
  ...
  >
    {#each tabItems as t (t.id || t)}
      <button
        data-id={t.id}
        class="flex-1 flex flex-col items-center gap-1 border-none bg-transparent cursor-pointer py-1 {state.tab ===
        t.id
          ? 'text-ios-blue'
          : 'text-ios-label2'}"
        onclick={setTab}
