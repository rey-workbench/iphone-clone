...
<div class="absolute inset-0 z-20 flex flex-col px-6 pt-12 pb-8">
  <!-- Drag Handle -->
  <div
    class="w-full flex justify-center mb-8 cursor-pointer"
    role="button"
    tabindex="0"
    onclick={() => (state.showPlayer = false)}
    onkeydown={(e) => { if (e.key === "Enter" || e.key === " ") state.showPlayer = false; }}
  >
  ...
  {#if state.showLyrics}
    ...
    >
      ...
      {:else if state.isSynced && state.parsedLyrics.length > 0}
        <div class="space-y-6 py-[40vh]">
          {#each state.parsedLyrics as line, i}
            ...
            <p
              class="transition-all duration-300 ease-out cursor-pointer {i === state.activeLyricIndex
      ...
      {:else}
        <div class="whitespace-pre-wrap py-[10vh] text-[20px] font-semibold text-white/60 text-center leading-relaxed">
          <span class="text-xs font-normal text-white/30 uppercase tracking-widest mb-6 block">Synced lyrics unavailable</span>
          {state.lyricsText}
        </div>
      {/if}
    </div>
  ...
  <div class="w-full flex-1 flex flex-col">
    ...
    <div class="w-full mb-10">
      ...
      <div class="flex justify-between mt-2.5 text-[11px] text-white/60 font-medium pointer-events-none">
        <span>{formatTime((state.progress / 100) * (state.player?.getDuration() || 0))}</span>
        <span>{formatTime((state.player?.getDuration() || 0) - (state.progress / 100) * (state.player?.getDuration() || 0), true)}</span>
      </div>
    </div>
    ...
    <div class="flex items-center gap-3 text-white/50 mb-10">
      ...
      <div
        ...
        tabindex="0"
      >
        <div class="absolute top-0 left-0 h-full bg-white rounded-full pointer-events-none" style="width: {state.volume}%"></div>
        <div class="absolute top-1/2 -translate-y-1/2 w-2 h-2 bg-white rounded-full shadow-md scale-[1.5] pointer-events-none" style="left: calc({state.volume}% - 4px)"></div>
      </div>
      <Volume2 size={20} />
    </div>
    ...
    <div class="flex items-center justify-evenly text-white/70 px-4 mt-auto">
      <button
        class="bg-transparent border-none cursor-pointer {state.showLyrics ? 'text-ios-pink' : 'text-current hover:text-white'}"
      ...
      <button class="bg-transparent border-none text-current cursor-pointer hover:text-white active:text-ios-pink" onclick={() => state.fetchUpNext()}><ListMusic size={22} /></button>
    </div>
  </div>
</div>
