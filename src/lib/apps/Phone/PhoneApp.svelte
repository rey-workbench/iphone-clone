    ...
    {#if state.tab === 'keypad'}
      <div class="flex flex-col items-center py-5">
        ...
        <div class="flex flex-col gap-3.5 py-5">
          {#each state.keys as row}
            <div class="flex gap-[22px]">
              {#each row as k}
                <button class="w-[78px] h-[78px] rounded-full bg-ios-fill border-none cursor-pointer flex flex-col items-center justify-center text-white active:bg-ios-fill/60 transition-colors" onclick={() => state.appendNumber(k.n)}>
        ...
        <div class="flex items-center justify-center gap-15 py-5">
          ...
          {#if state.dialNumber}
            <button class="bg-transparent border-none text-2xl cursor-pointer text-ios-label2" onclick={() => state.backspace()} aria-label="Backspace">
              <Delete size={28} />
            </button>
          {:else}
            <span class="w-6"></span>
