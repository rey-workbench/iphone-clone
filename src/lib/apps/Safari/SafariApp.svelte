<script lang="ts">
	...
	import { ChevronLeft, ChevronRight, Share, BookOpen, Copy, Lock } from '@lucide/svelte';
...
>
	{#if state.showInput}
		...
		<div class="flex items-center gap-2 px-4 py-3 bg-[#f2f2f6] z-20">
			<div class="flex-1 h-10 rounded-[10px] bg-[#e3e3e8] flex items-center px-3">
				<input
	...
	>
			...
			<div class="p-4 flex flex-col gap-6 bg-white min-h-full">
				{#each state.searchResults as result, i (i)}
					...
					>
						...
						<div class="text-[18px] text-[#1a0dab] hover:underline font-medium leading-tight">
							{result.title}
						</div>
						<div class="text-[14px] text-[#4d5156] leading-snug line-clamp-3 mt-1">
				...
				<div class="grid grid-cols-4 gap-y-6 gap-x-2">
					{#each favorites as fav (fav.url)}
						...
						>
							...
							<span class="text-[11px] text-gray-500 truncate w-full text-center">{fav.name}</span>
						</button>
					{/each}
				</div>
			</div>
		{:else}
			{#if state.errorMessage}
				<div class="p-6 text-red-500 flex items-center justify-center h-full text-center font-bold">
					{state.errorMessage}
				</div>
	...
	{#if !state.showInput}
		...
		>
			...
			<div class="flex items-center justify-between px-5 pt-1 pb-1">
				...
				<button class="text-ios-blue bg-transparent border-none p-2 cursor-pointer">
					<Share size={24} />
				</button>
				<button class="text-ios-blue bg-transparent border-none p-2 cursor-pointer">
					<BookOpen size={24} />
