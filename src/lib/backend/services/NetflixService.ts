...
export class NetflixService {
	...
	private formatItem(item: any, type?: string) {
		return {
			...
			backdrop_path: item.backdrop_path
				? `${ApiConfig.TMDB_IMAGE_BASE}/w1280${item.backdrop_path}`
				: null,
	...
	async getDetails(id: string, type: string) {
		...
		const res = await fetch(
			`${ApiConfig.TMDB_API_BASE}/${type}/${id}?append_to_response=credits,videos`,
			this.tmdbOptions
		);
	...
	async getLatest() {
		...
		const fetchTmdbDetails = async (item: any, type: 'movie' | 'tv') => {
			...
			try {
				const res = await fetch(
					`${ApiConfig.TMDB_API_BASE}/${type}/${item.tmdb_id}?language=en-US`,
					this.tmdbOptions
				);
				const data = await res.json();
				if (data.id) {
	...
	async search(query: string) {
		...
		const res = await fetch(
			`${ApiConfig.TMDB_API_BASE}/search/multi?query=${encodeURIComponent(query)}&include_adult=false&language=en-US&page=1`,
			this.tmdbOptions
		);
		const data = await res.json();
	...
	async getTrending() {
		...
		const moviesRes = await fetch(
			`${ApiConfig.TMDB_API_BASE}/trending/movie/day?language=en-US`,
			this.tmdbOptions
		);
		const moviesData = await moviesRes.json();

		const tvRes = await fetch(
			`${ApiConfig.TMDB_API_BASE}/trending/tv/day?language=en-US`,
			this.tmdbOptions
		);
		const tvData = await tvRes.json();
