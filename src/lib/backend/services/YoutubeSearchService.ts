...
export class YoutubeSearchService {
	...
	async getLyrics(
		...
		if (title && artist) {
			try {
				...
				if (duration) {
					try {
						match = await lrcClient.findLyrics({
							track_name: title,
							artist_name: artist,
							duration: parseInt(duration)
						});
					} catch {
						/* duration match failed, try general search */
					}
				}
				...
				/* lrclib not available, fallback to ytmusic */
			}
		}
		...
		const results = upnext.map((s: any) => ({
			...
			artist:
				typeof s.artists === 'string'
					? s.artists
					: s.artists?.map?.((a: any) => a.name).join(', ') || 'Unknown',
	...
	async searchOrBrowse(query: string | null, action: string | null) {
			...
			sections.forEach((section: any) => {
				section.contents?.forEach((item: any) => {
					if (
						item &&
						item.type &&
						['SONG', 'VIDEO', 'PLAYLIST', 'ALBUM', 'ARTIST'].includes(item.type)
					) {
						songs.push(item);
					}
				});
		...
		const results = songs.slice(0, 40).map((s: any) => ({
			...
			artist:
				s.artist?.name ||
				(s.type === 'PLAYLIST' ? 'Playlist' : s.type === 'ARTIST' ? 'Artist' : 'YouTube'),
			art: s.thumbnails?.[s.thumbnails.length - 1]?.url || '',
			preview: `https://music.youtube.com/watch?v=${s.videoId}`,
