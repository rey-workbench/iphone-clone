...
<div class="absolute inset-0 z-60 bg-white flex flex-col font-sans">
	...
	<div class="w-full flex items-center px-2 h-11">
		<button class="flex items-center text-ios-gray3 bg-transparent border-none text-[17px]">
			<ChevronLeft size={24} />
			<span>Back</span>
		</button>
	</div>
	...
	>
		...
		<!-- Inputs Box -->
		<div class="w-full bg-ios-gray6 rounded-[10px] overflow-hidden mb-4">
			<input
				id="username"
				name="username"
				type="text"
			...
			<input
				...
				class="w-full h-12.5 px-4 text-[17px] text-black bg-transparent border-none outline-none placeholder:text-ios-gray disabled:opacity-50"
			/>
		</div>

		{#if authGlobalState.errorMsg}
			<div class="text-ios-red text-[13px] text-center mb-4 px-4 animate-[fadeIn_0.3s_ease-out]">
				{authGlobalState.errorMsg}
			</div>
		{/if}

		<button class="bg-transparent border-none text-ios-gray text-[15px] cursor-pointer mb-auto">
		...
		<div class="flex flex-col items-center mt-12 mb-8">
			...
			<p class="text-[11px] text-ios-gray text-center leading-tight">
		...
		<!-- Bottom link -->
		<button class="bg-transparent border-none text-ios-gray text-[15px] cursor-pointer pb-4">
