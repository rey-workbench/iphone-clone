...
{#if shellState.isControlCenterOpen || shellState.isControlCenterDragging}
	...
	>
		<!-- Background Blur -->
		<div
			class="absolute inset-0 bg-black/30 backdrop-blur-3xl animate-[fadeIn_0.3s_ease-out]"
		></div>

		<!-- Layout Grid -->
		<div
			class="relative z-10 p-6 pt-16 h-full flex flex-col gap-4 animate-[slideDown_0.35s_cubic-bezier(0.23,1,0.32,1)]"
			style:animation={!shellState.isControlCenterOpen && !shellState.isControlCenterDragging
				? 'none'
				: null}
		>
			<!-- Top Section -->
			<div class="flex gap-4 h-44">
				<!-- Network Block -->
				<div
					class="flex-1 bg-white/10 backdrop-blur-xl rounded-[28px] p-4 flex flex-col justify-between shadow-lg"
				>
				...
				<div class="flex-1 bg-white/10 backdrop-blur-xl rounded-[28px] p-4 shadow-lg flex flex-col">
					<div class="flex justify-end">
						<button
							class="p-1 rounded-full bg-white/20 border-none"
							aria-label="Maximize Media Controls"><Maximize2 color="white" size={16} /></button
						>
					</div>
					...
					<div class="flex justify-center gap-6 pb-2">
						<button class="bg-transparent border-none" aria-label="Previous Track"
							><Rewind color="white" size={22} fill="white" /></button
						>
						<button class="bg-transparent border-none" aria-label="Play/Pause"
							><Play color="white" size={26} fill="white" /></button
						>
						<button class="bg-transparent border-none" aria-label="Next Track"
							><Forward color="white" size={22} fill="white" /></button
						>
					</div>
				</div>
			</div>
			...
			<div class="flex gap-4 h-40">
				...
				<div class="flex-1 flex flex-col gap-4">
					<div class="flex gap-4 flex-1">
						<button
							class="flex-1 bg-white backdrop-blur-xl rounded-[20px] flex items-center justify-center border-none shadow-lg text-ios-red"
							aria-label="Rotation Lock"><Lock size={28} /></button
						>
						<button
							class="flex-1 bg-white/10 backdrop-blur-xl rounded-[20px] flex items-center justify-center border-none shadow-lg text-white"
							aria-label="Screen Mirroring"><Copy size={28} /></button
						>
					</div>
					<div
						class="bg-white/10 backdrop-blur-xl rounded-[20px] p-3 flex items-center gap-3 shadow-lg flex-1 border-none justify-start"
					>
						<div
							class="w-9 h-9 rounded-full bg-ios-indigo flex items-center justify-center shrink-0"
						>
			...
			<div class="grid grid-cols-4 gap-4 mt-2">
				<button
					class="aspect-square bg-white/10 backdrop-blur-xl rounded-[20px] flex items-center justify-center border-none shadow-lg text-white"
					aria-label="Flashlight"><Flashlight size={28} /></button
				>
				<button
					class="aspect-square bg-white/10 backdrop-blur-xl rounded-[20px] flex items-center justify-center border-none shadow-lg text-white"
					aria-label="Timer"><AlarmClock size={28} /></button
				>
				<button
					class="aspect-square bg-white/10 backdrop-blur-xl rounded-[20px] flex items-center justify-center border-none shadow-lg text-white"
					aria-label="Calculator"><Calculator size={28} /></button
				>
				<button
					class="aspect-square bg-white/10 backdrop-blur-xl rounded-[20px] flex items-center justify-center border-none shadow-lg text-white"
					aria-label="Camera"><Camera size={28} /></button
				>
				...
				>
					<svg
						...
						><path
							d="M3 7v10a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-3l-2-2H8L6 7H5a2 2 0 0 0-2 2z"
				...
				<button
					class="aspect-square bg-white/10 backdrop-blur-xl rounded-[20px] flex items-center justify-center border-none shadow-lg text-white"
					aria-label="Low Power Mode"><Battery size={28} /></button
				>
				<button
					class="aspect-square bg-white/10 backdrop-blur-xl rounded-[20px] flex items-center justify-center border-none shadow-lg text-white"
					aria-label="Notes"
