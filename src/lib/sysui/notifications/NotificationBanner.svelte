...
<div
	class="absolute top-10 left-0 right-0 z-100000 flex flex-col items-center gap-2 pointer-events-none px-4"
>
	{#each notificationGlobalState.notifications as notif (notif.id)}
		...
		>
			{#if notif.icon}
				<div
					class="w-10 h-10 shrink-0 rounded-[10px] overflow-hidden flex items-center justify-center bg-black/30 shadow-sm"
				>
